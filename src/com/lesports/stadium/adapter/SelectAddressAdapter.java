...
public class SelectAddressAdapter extends BaseAdapter{
	 ...
	 */
	private String id="";
	public SelectAddressAdapter(Context mContext,
			List<ShippingAddressBean> addressList,String id) {
		super();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(Utils.isNullOrEmpty(id)){
			if(position==0){
				...
				holder.selectedTb.setChecked(false);
			}
		}else{
			if(addressList.get(position).getId().equals(id)){
				holder.selectedTb.setChecked(true);
			}else{
				holder.selectedTb.setChecked(false);
			}
