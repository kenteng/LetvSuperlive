...
public class RopeProgressBarend extends View {
    ...
    public RopeProgressBarend(final Context context) {
        this(context, null);
    }

    public RopeProgressBarend(final Context context, final AttributeSet attrs) {
    ...
    public RopeProgressBarend(
        ...
        mBubblePaint.setColor(Color.rgb(114, 114, 114));
...
//      Math.max(0, progressEnd - (getTriangleWidth() / 2) - bubbleLeft));
    ...
    public String getIndexCurrent(){
    	float index=0;
    	if(CurrentProgress==0||CurrentTotal==0){
    		index=0;
    	}else{
    		index=(float)CurrentProgress/(float)CurrentTotal;
    	}
    	//已经结束
		return (index*100)+"%";
