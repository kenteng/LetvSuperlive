...
import com.lesports.stadium.view.CarTimeSelector;
 ...
 * @author wangxinnian
 * 
 * @date 2016-7-11 上午9:32:28
 * 
 * 
 */
public class UserCarActivity extends Activity implements OnClickListener {
	/**
	 * 上下文
	 */
	...
	// "psTBKnSYPotjmpj0rRillUPhDdJ2S7CQEDRlyiKJ";
	/**
	 * 选择时间,乘车时间
	 */
		...
		public void handleMessage(android.os.Message msg) {
			...
			case HANDLE_TAG_0:
				...
				if(!TextUtils.isEmpty(backdata)){
					useWayHandlegetprice(backdata);
				}
				break;
			case HANDLE_TAG_1:
				// 创建订单
				String orderBackdata = (String) msg.obj;
				if(!TextUtils.isEmpty(orderBackdata)){
					useWayHandleOrder(orderBackdata);
				}
				break;
			case HANDLE_TAG_2:
				// 向我们服务器发送订单号完成
				isGoRide = true;
				Intent bookCarIntent = new Intent(UserCarActivity.this,
						BookthecarActivity.class);
				bookCarIntent.putExtra("orderid", LApplication.yidaoOrderId);
				bookCarIntent.putExtra("tag", "service");
				UserCarActivity.this.startActivity(bookCarIntent);
				break;
			default:
				break;
			}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.layout_carview);
		context = this;
		initBaseData();
		initView();
		initData();
	...
	public void initView() {
		...
		timeSelector = new CarTimeSelector(UserCarActivity.this,
		...
		// 选车背景宽度
		int bgWidth = GlobalParams.WIN_WIDTH - DensityUtil.dip2px(UserCarActivity.this, 20)
				- DensityUtil.dip2px(UserCarActivity.this, 70);
		firstPoint.setX(0);
			...
			if (isGoRide) {
				...
				if (Utils.isNullOrEmpty(GlobalParams.USER_ID)) {
					...
					if (Utils.isNullOrEmpty(rideAddress)
							...
							|| Utils.isNullOrEmpty(mRideAddressTv.getText()
									.toString())) {
						showDiaLog("请补全用车信息");
					} else {
						isGoRide = false;
	...
	private void requestPrices(YongCheBean carBean) {
		if (Utils.isNullOrEmpty(rideAddress)
			...
			params.put("expect_start_longitude",
					"" + rideAddress.getLongitude()); // 出发地经度
			params.put("expect_start_latitude", "" + rideAddress.getLatitude()); // 出发地纬度
			params.put("expect_end_longitude",
					"" + getoffAddress.getLongitude()); // 目的地经度
	...
	private void useWayHandlegetprice(String backdata) {
		...
		try {
			...
			if (priceObj.toString().contains("result")) {
				...
				if (!Utils.isNullOrEmpty(errorStr)) {
					// Toast.makeText(context, ""+errorStr,
					// Toast.LENGTH_SHORT).show();
					tv_price.setText("30");
	 ...
	 * @param orderBackdata
	 */
	private void useWayHandleOrder(String orderBackdata) {
		// TODO Auto-generated method stub
		try {
			...
			if (orderObj.toString().contains("result")) {
				 ...
				 * context.startActivity(bookCarIntent);
				 */
				MobclickAgent.onEvent(UserCarActivity.this,
						"OrderCarSuccessfully");
			} else {
				isGoRide = true;
				Toast.makeText(UserCarActivity.this,
						orderObj.getString("msg"), Toast.LENGTH_SHORT)
						.show();
			}
		} catch (JSONException e) {
			e.printStackTrace();
