...
import com.lesports.stadium.lsyvideo.ui.utils.PxUtils;
...
public class WaterMarkView extends RelativeLayout {
	...
	public void setWaterMarks(List<WaterConfig> marks) {
		...
		for (WaterConfig waterConfig : marks) {
			WaterMarkImageView waterMark = new WaterMarkImageView(mContext,
					waterConfig.getPicUrl());
			int pos = 1;
			try {
	...
	private RelativeLayout.LayoutParams getWatermarkLocation(int location) {
		RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
				PxUtils.dip2px(mContext, 40), PxUtils.dip2px(mContext, 26));
		switch (location) {
		case 1: // 左上角
