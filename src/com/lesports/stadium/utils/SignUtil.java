	...
	public static String createSign( Map<String, String> params){
		...
		String result="";
		 if (params != null && !params.isEmpty()) {
			for (Map.Entry<String, String> entry : params
				...
				if(entry.getValue()!=null){
					if(!entry.getValue().toString().equals("")){
						if(!entry.getKey().equals("requestTime")){
			...
			//Key 排序
			List<String> newList=listSort(list);
			...
			if (result.equals("")) {
				result =result+"appsecret="+"letvsecret";
			} else {
				result =result+"&appsecret="+"letvsecret";
			}
				
			result =result+ "&ntime="+params.get("requestTime");
//			result=(CommonUtils.md5(result)).toUpperCase();
			result=CommonUtils.encryption(result);
		}		 
		return result;
	}
