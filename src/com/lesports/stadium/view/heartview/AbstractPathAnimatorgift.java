 ...
 * ***************************************************************
 */
public abstract class AbstractPathAnimatorgift {
    private final Random mRandom;
    protected final Config mConfig;


    public AbstractPathAnimatorgift(Config config) {
        mConfig = config;
        mRandom = new Random();
    }
    ...
    public Path createPath(AtomicInteger counter, View view, int factor) {
        ...
        y2 = y - y2 / 2;
        Path p = new Path();
        p.moveTo(mConfig.initX, y3);
        p.cubicTo(mConfig.initX, y2 +factor, x, y- factor, x, y2);
        p.moveTo(x, y2);
        p.cubicTo(x, y, x2, y2 - factor, x2, y);
        return p;
    }
